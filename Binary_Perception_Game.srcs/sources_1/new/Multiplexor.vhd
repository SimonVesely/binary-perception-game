-- Component functioning as multiplexor between target_value/timer/user_input
    ...
    process(cnt, user_u, user_t, user_h, left_u, left_t, left_h, show_time)
     begin
        ...
        case cnt is
            ...
            when "001" => an <= "11111101"; mux_out <= user_t; if show_time = '1' then dp <= '0'; end if;
            when "010" => an <= "11111011"; mux_out <= user_h;
            when "101" => an <= "11011111"; mux_out <= left_u;
            when "110" => an <= "10111111"; mux_out <= left_t;
